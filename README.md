...
| `BOT_STATS_TOKEN` | нет | Bearer-токен приватного `GET /internal/trigger-stats` (число командных уникальных триггеров по ISO-неделям для локального дашборда). Не задан → маршрут не регистрируется |
